...
if (isset($_POST['submit'])) {
    ...
    $cek = $koneksi->query("SELECT * FROM produk WHERE namaproduk='$idproduk'");
                                                ...
                                                <div class="row mt-3 mb-3">
                                                        ...
                                                        <div class="form-group">
                                                            ...
                                                            <select name="idproduk" class="form-control">
                                                                ...
                                                                <?php while ($pecah = $ambil->fetch_assoc()) { ?>
                                                                    <option <?php if ($idproduk == $pecah['namaproduk']) echo 'selected'; ?> value="<?= $pecah['namaproduk'] ?>"><?= $pecah['namaproduk'] ?></option>
                                                                <?php } ?>
                                                            </select>
                                                        </div>
                                                    </div>
                                                    <div class="col-md-3">
                                                        <div class="form-group mb-3">
                                                            ...
                                                            <a target="_blank" href="laporanpenjualancetak.php?tahun=<?= $tahun ?>&bulan=<?= $bulan ?>&idproduk=<?= $idbarang ?>" class="btn btn-success text-white" style="margin-top:30px">Cetak</a>
                                                        </div>
                                                    </div>
                                                </div>
                                            </form>
                                            <table class="table table-bordered table-striped" id="table">
                                                ...
                                                <tbody>
                                                    ...
                                                    if (isset($_POST['submit'])) {
                                                        ...
                                                        $idproduk = $_POST['idproduk'];
                                                        if ($idproduk == "") {
                                                            $ambil = $koneksi->query("SELECT*FROM penjualan join produk on penjualan.idproduk = produk.idproduk WHERE year(tanggalpenjualan) = '$tahun' and month(tanggalpenjualan) = '$bulan' order by tanggalpenjualan desc") or die(mysqli_error($koneksi));
                                                        } else {
                                                            $ambil = $koneksi->query("SELECT*FROM penjualan join produk on penjualan.idproduk = produk.idproduk WHERE year(tanggalpenjualan) = '$tahun' and month(tanggalpenjualan) = '$bulan' and idproduk = '$idproduk'  order by tanggalpenjualan desc") or die(mysqli_error($koneksi));
                                                        }
                                                    } else {
                                                        $ambil = $koneksi->query("SELECT*FROM penjualan join produk on penjualan.idproduk = produk.idproduk WHERE year(tanggalpenjualan) = '$tahun' and month(tanggalpenjualan) = '$bulan' order by tanggalpenjualan desc") or die(mysqli_error($koneksi));
                                                    }
                                                    ...
                                                    <?php while ($pecah = $ambil->fetch_assoc()) { ?>
                                                        ...
                                                        <?php
                                                        $totalpemasukan += $pecah['harga'] * $pecah['jumlah'];
                                                        $nomor++; ?>
                                                    <?php } ?>
                                                </tbody>
                                                <tfoot>
